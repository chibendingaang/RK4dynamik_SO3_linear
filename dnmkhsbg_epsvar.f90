...
!stores the output from Dynamik in the files for spin_[a-b]
do i=0,nprocs-1
  if(rank==i)then
!   start = MPI_Wtime()

!check the value for epsil for cofig_b before deciding the folder
   open(176,file='./L512/eps_min3/qwhsbg/spin_a_'//trim(adjustl(seedchar))//'.dat',status='new')
   write(176,*)spin_a
   close(176)

!goto 16
   open(189,file='./L512/eps_min3/qwhsbg/spin_b_'//trim(adjustl(seedchar))//'.dat',status='new')
   write(189, *) spin_b
   close(189)
16 continue
