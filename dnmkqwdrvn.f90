...
do i=0,nprocs-1
  if(rank==i)then
!   start = MPI_Wtime()
   open(176,file='./qwdrvn/5emin3/spin_a_'//trim(adjustl(seedchar))//'.dat',status='new')
   write(176,*)spin_a
   close(176)

!goto 16
   open(189,file='./qwdrvn/5emin3/spin_b_'//trim(adjustl(seedchar))//'.dat',status='new')
   write(189, *) spin_b
   close(189)
16 continue
